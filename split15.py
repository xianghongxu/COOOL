...
def main():
    ...
    for index, (fp, q) in enumerate(queries):
        ...
        # fp_name = fp.split('.')[0]
        print(fp)
        if (not file_exist(saved_files, fp)):
            plans = []
            latencys = []
            for i in range(NUM_HINT_SET):
                ...
                if str(plan) in exe_plan_dict.keys():
                    ...
                    # print(f'query {index+1} hint {i} hit')
                else:
...
if __name__ == '__main__':
    ...
    for index, (fp, q) in enumerate(queries):
        q_name = fp.split('.')[0]
        whole_query = q
        whole_query = whole_query.split('\n\n')
        print(len(whole_query))
        new_name = q_name + "step"
        save_path = os.path.join(args.data_root_path, args.dataset, '15')
        mkdir(save_path)
        file_sql_path = os.path.join(save_path, f'{new_name}_0.sql')
        with open(file_sql_path, 'w') as f:
            f.write(whole_query[0])
            f.write('\n')
            f.write(whole_query[1])
        file_sql_path = os.path.join(save_path, f'{new_name}_1.sql')
        with open(file_sql_path, 'w') as f:
            f.write(whole_query[2])
        file_sql_path = os.path.join(save_path, f'{new_name}_2.sql')
        with open(file_sql_path, 'w') as f:
            f.write(whole_query[3])
